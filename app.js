...
(() => {
  ...
  function showAuth() { $('auth')?.classList.remove('hidden'); $('app')?.classList.add('hidden'); closeMenu?.(); $('bottomNav')?.classList.add('auth-hidden'); }
  function showApp() { $('auth')?.classList.add('hidden'); $('app')?.classList.remove('hidden'); $('bottomNav')?.classList.remove('auth-hidden'); }
  ...
  function go(page){
    ...
    if(!view){ toast('Modul '+page+' belum tersedia.',false); return false; }
    ...
    const titleNode=title?.querySelector('b') || title?.querySelector('span');
    if($('pageTitle')) $('pageTitle').textContent=(titleNode?.textContent || title?.textContent || page).replace(/[›↳]/g,'').trim();
    closeMenu();
  ...
  function bind(){
    ...
    $('salesProducts')?.addEventListener('click',()=>go('stock'));
    $('topSales')?.addEventListener('click',()=>go('sales'));
    $('topPurchases')?.addEventListener('click',()=>go('purchases'));
    document.querySelectorAll('.bottom-item[data-page], .nav-item[data-page], .menu-subitem[data-page]').forEach(b=>b.addEventListener('click',()=>go(b.dataset.page)));
