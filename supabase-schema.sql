...
-- ---------- V3 OPENING BALANCE ADJUSTMENT ----------
-- Menambah/mengurangi saldo awal tanpa menghapus histori transaksi.
...
begin
  if auth.uid() is null then raise exception 'AUTH_REQUIRED'; end if;
  if p_amount is null or p_amount <= 0 then raise exception 'Nominal harus lebih dari 0'; end if;
  if p_action not in ('add','subtract') then raise exception 'Aksi saldo awal tidak valid'; end if;
  select * into c from public.cash_accounts where id=p_cash_account_id and active for update;
  if not found then raise exception 'Kas/Bank tidak ditemukan'; end if;
  old_balance:=coalesce(c.opening_balance,0);
  new_balance:=case when p_action='add' then old_balance+p_amount else old_balance-p_amount end;
  if new_balance < 0 then raise exception 'Saldo awal tidak boleh negatif'; end if;
  cash_code:=case when c.account_type='bank' then '1200' else '1100' end;
  if not exists(select 1 from public.accounts where code=cash_code and active) then raise exception 'Akun Kas/Bank belum tersedia'; end if;
  update public.cash_accounts set opening_balance=new_balance where id=c.id returning * into c;
  select coalesce(max((regexp_match(journal_no,'([0-9]+)$'))[1]::int),0)+1 into n from public.journal_headers;
  insert into public.journal_headers(journal_no,journal_date,journal_type,source_type,source_id,description,status,created_by)
  values('JRN-'||lpad(n::text,5,'0'),coalesce(p_date,current_date),'general','opening_balance',c.id,coalesce(p_description,'Penyesuaian saldo awal'),'posted',auth.uid()) returning * into j;
  if p_action='add' then
    insert into public.journal_lines(journal_id,line_no,account_id,description,debit,credit)
    values(j.id,1,(select id from public.accounts where code=cash_code),j.description,p_amount,0),
          (j.id,2,(select id from public.accounts where code='3100'),j.description,0,p_amount);
  else
    insert into public.journal_lines(journal_id,line_no,account_id,description,debit,credit)
    values(j.id,1,(select id from public.accounts where code='3100'),j.description,p_amount,0),
          (j.id,2,(select id from public.accounts where code=cash_code),j.description,0,p_amount);
  end if;
  insert into public.audit_logs(user_id,action,table_name,record_id,old_data,new_data)
  values(auth.uid(),'update','cash_accounts',c.id,to_jsonb(jsonb_build_object('opening_balance',old_balance)),to_jsonb(c));
  return c;
end $$;

revoke all on function public.post_opening_balance_adjustment(date,uuid,text,numeric,text) from public;
grant execute on function public.post_opening_balance_adjustment(date,uuid,text,numeric,text) to authenticated;

-- ============================================================
-- KOREKSI DATA / EDIT TRANSAKSI
-- Semua koreksi lewat RPC agar transaksi + jurnal tetap sinkron.
-- ============================================================
create or replace function public.update_cash_transaction(
  p_id uuid, p_date date, p_direction text, p_category text, p_amount numeric,
  p_cash_account_id uuid, p_description text, p_reference text default null, p_pic text default null
) returns public.transactions
language plpgsql security definer set search_path=public
as $$
...
begin
  ...
  if p_direction not in ('in','out') then raise exception 'Jenis transaksi tidak valid'; end if;
  if p_cash_account_id is null then raise exception 'Kas/Bank wajib dipilih'; end if;
  select * into r from transactions where id=p_id for update;
  if not found then raise exception 'Transaksi tidak ditemukan'; end if;
  if r.status='void' then raise exception 'Transaksi void tidak dapat diedit'; end if;
  select case when account_type='bank' then '1200' else '1100' end into cash_code from cash_accounts where id=p_cash_account_id and active;
  if cash_code is null then raise exception 'Kas/Bank tidak ditemukan'; end if;
  if p_direction='in' then
    contra_code := case p_category when 'Penjualan' then '4100' when 'Modal' then '3100' when 'Pelunasan Piutang' then '1300' else '4200' end;
    jtype := 'receipt';
  else
  ...
  update transactions set
    transaction_date=coalesce(p_date,current_date), category=p_category,
    description=coalesce(nullif(trim(p_description),''),'Transaksi'),
    cash_account_id=p_cash_account_id,
    cash_in=case when p_direction='in' then p_amount else 0 end,
    cash_out=case when p_direction='out' then p_amount else 0 end,
    reference_no=nullif(trim(coalesce(p_reference,'')),''), pic=nullif(trim(coalesce(p_pic,'')),''), updated_at=now()
  where id=p_id returning * into r;

  select * into j from journal_headers where source_type='transaction' and source_id=r.id order by created_at desc limit 1 for update;
  if found then
    update journal_headers set journal_date=r.transaction_date,journal_type=jtype,description=r.description where id=j.id;
    delete from journal_lines where journal_id=j.id;
  else
    select coalesce(max((regexp_match(journal_no,'([0-9]+)$'))[1]::int),0)+1 into n from journal_headers;
    insert into journal_headers(journal_no,journal_date,journal_type,source_type,source_id,description,status,created_by)
    values('JRN-'||lpad(n::text,5,'0'),r.transaction_date,jtype,'transaction',r.id,r.description,'posted',auth.uid()) returning * into j;
  end if;
  if p_direction='in' then
    insert into journal_lines(journal_id,line_no,account_id,description,debit,credit)
    values(j.id,1,(select id from accounts where code=cash_code),r.description,p_amount,0),
          (j.id,2,(select id from accounts where code=contra_code),r.description,0,p_amount);
  else
    insert into journal_lines(journal_id,line_no,account_id,description,debit,credit)
    values(j.id,1,(select id from accounts where code=contra_code),r.description,p_amount,0),
          (j.id,2,(select id from accounts where code=cash_code),r.description,0,p_amount);
  end if;
  insert into audit_logs(user_id,action,table_name,record_id,new_data) values(auth.uid(),'update','transactions',r.id,to_jsonb(r));
  return r;
end $$;
revoke all on function public.update_cash_transaction(uuid,date,text,text,numeric,uuid,text,text,text) from public;
grant execute on function public.update_cash_transaction(uuid,date,text,text,numeric,uuid,text,text,text) to authenticated;

create or replace function public.update_product(
  p_id uuid, p_sku text, p_name text, p_size text, p_fabric_type text,
  p_selling_price numeric, p_reorder_level numeric, p_active boolean default true
...
begin
  if auth.uid() is null then raise exception 'AUTH_REQUIRED'; end if;
  if p_selling_price is null or p_selling_price < 0 then raise exception 'Harga jual tidak valid'; end if;
  if p_reorder_level is null or p_reorder_level < 0 then raise exception 'Batas reorder tidak valid'; end if;
  update products set sku=trim(p_sku),name=trim(p_name),size=nullif(trim(coalesce(p_size,'')),''),fabric_type=nullif(trim(coalesce(p_fabric_type,'')),''),selling_price=p_selling_price,reorder_level=p_reorder_level,active=coalesce(p_active,true)
  where id=p_id returning * into r;
  if not found then raise exception 'Produk tidak ditemukan'; end if;
  insert into audit_logs(user_id,action,table_name,record_id,new_data) values(auth.uid(),'update','products',r.id,to_jsonb(r));
  return r;
end $$;
revoke all on function public.update_product(uuid,text,text,text,text,numeric,numeric,boolean) from public;
grant execute on function public.update_product(uuid,text,text,text,text,numeric,numeric,boolean) to authenticated;

-- ---------- KOREKSI PENJUALAN ----------
create or replace function public.update_sale(
  p_id uuid, p_date date, p_customer text, p_total numeric, p_paid numeric, p_due date, p_cash_account_id uuid
) returns public.sales
language plpgsql security definer set search_path=public
as $$
declare
  r public.sales; j public.journal_headers; remain numeric; cash_code text; ln int:=1;
begin
  if auth.uid() is null then raise exception 'AUTH_REQUIRED'; end if;
  if p_total is null or p_total<=0 or p_paid<0 or p_paid>p_total then raise exception 'Nilai penjualan tidak valid'; end if;
  if p_paid>0 and p_cash_account_id is null then raise exception 'Kas/Bank wajib dipilih jika ada pembayaran'; end if;
  select * into r from sales where id=p_id for update;
  if not found then raise exception 'Penjualan tidak ditemukan'; end if;
  remain:=p_total-p_paid;
  if p_paid>0 then select case when account_type='bank' then '1200' else '1100' end into cash_code from cash_accounts where id=p_cash_account_id and active; end if;
  update sales set sale_date=coalesce(p_date,current_date),customer_name=nullif(trim(coalesce(p_customer,'')),''),total=p_total,paid=p_paid,due_date=p_due,cash_account_id=p_cash_account_id,status=case when remain=0 then 'paid' when p_paid=0 then 'unpaid' else 'partial' end where id=p_id returning * into r;
  select * into j from journal_headers where source_type='sale' and source_id=r.id order by created_at desc limit 1 for update;
  if found then update journal_headers set journal_date=r.sale_date,description='Penjualan '||r.sale_no where id=j.id; delete from journal_lines where journal_id=j.id;
  else insert into journal_headers(journal_no,journal_date,journal_type,source_type,source_id,description,status,created_by) values('JRN-'||lpad((coalesce((select max((regexp_match(journal_no,'([0-9]+)$'))[1]::int) from journal_headers),0)+1)::text,5,'0'),r.sale_date,'sales','sale',r.id,'Penjualan '||r.sale_no,'posted',auth.uid()) returning * into j; end if;
  ...
  insert into journal_lines(journal_id,line_no,account_id,description,debit,credit) values(j.id,ln,(select id from accounts where code='4100'),j.description,0,p_total);
  delete from accounts_receivable where reference_no=r.sale_no;
  if remain>0 then insert into accounts_receivable(customer_name,reference_no,invoice_date,amount,paid,due_date,status) values(coalesce(r.customer_name,'Pelanggan'),r.sale_no,r.sale_date,p_total,p_paid,r.due_date,case when p_paid=0 then 'unpaid' else 'partial' end); end if;
  insert into audit_logs(user_id,action,table_name,record_id,new_data) values(auth.uid(),'update','sales',r.id,to_jsonb(r));
  return r;
end $$;
revoke all on function public.update_sale(uuid,date,text,numeric,numeric,date,uuid) from public;
grant execute on function public.update_sale(uuid,date,text,numeric,numeric,date,uuid) to authenticated;

-- ---------- KOREKSI PEMBELIAN ----------
create or replace function public.update_purchase(
  p_id uuid, p_date date, p_supplier text, p_total numeric, p_paid numeric, p_due date, p_cash_account_id uuid
) returns public.purchases
language plpgsql security definer set search_path=public
as $$
declare
  r public.purchases; j public.journal_headers; remain numeric; cash_code text; ln int:=1;
begin
  if auth.uid() is null then raise exception 'AUTH_REQUIRED'; end if;
  if p_total is null or p_total<=0 or p_paid<0 or p_paid>p_total then raise exception 'Nilai pembelian tidak valid'; end if;
  if p_paid>0 and p_cash_account_id is null then raise exception 'Kas/Bank wajib dipilih jika ada pembayaran'; end if;
  select * into r from purchases where id=p_id for update;
  if not found then raise exception 'Pembelian tidak ditemukan'; end if;
  remain:=p_total-p_paid;
  if p_paid>0 then select case when account_type='bank' then '1200' else '1100' end into cash_code from cash_accounts where id=p_cash_account_id and active; end if;
  update purchases set purchase_date=coalesce(p_date,current_date),supplier_name=nullif(trim(coalesce(p_supplier,'')),''),total=p_total,paid=p_paid,due_date=p_due,cash_account_id=p_cash_account_id,status=case when remain=0 then 'paid' when p_paid=0 then 'unpaid' else 'partial' end where id=p_id returning * into r;
  select * into j from journal_headers where source_type='purchase' and source_id=r.id order by created_at desc limit 1 for update;
  if found then update journal_headers set journal_date=r.purchase_date,description='Pembelian '||r.purchase_no where id=j.id; delete from journal_lines where journal_id=j.id;
  else insert into journal_headers(journal_no,journal_date,journal_type,source_type,source_id,description,status,created_by) values('JRN-'||lpad((coalesce((select max((regexp_match(journal_no,'([0-9]+)$'))[1]::int) from journal_headers),0)+1)::text,5,'0'),r.purchase_date,'purchase','purchase',r.id,'Pembelian '||r.purchase_no,'posted',auth.uid()) returning * into j; end if;
  ...
  delete from accounts_payable where reference_no=r.purchase_no;
  if remain>0 then insert into accounts_payable(supplier_name,reference_no,invoice_date,amount,paid,due_date,status) values(coalesce(r.supplier_name,'Supplier'),r.purchase_no,r.purchase_date,p_total,p_paid,r.due_date,case when p_paid=0 then 'unpaid' else 'partial' end); end if;
  insert into audit_logs(user_id,action,table_name,record_id,new_data) values(auth.uid(),'update','purchases',r.id,to_jsonb(r));
  return r;
end $$;
revoke all on function public.update_purchase(uuid,date,text,numeric,numeric,date,uuid) from public;
grant execute on function public.update_purchase(uuid,date,text,numeric,numeric,date,uuid) to authenticated;
